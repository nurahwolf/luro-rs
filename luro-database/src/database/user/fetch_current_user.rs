...
impl crate::Database {
    ...
    pub async fn user_fetch_current_user(&self, guild_id: Option<Id<GuildMarker>>) -> anyhow::Result<User> {
        match guild_id {
            Some(guild_id) => if let Ok(current_user) = self.member_fetch(self.current_user, guild_id).await {
                return Ok(current_user)
            },
            None => if let Ok(current_user) = self.user_fetch(self.current_user).await {
                return Ok(current_user)
            },
        }
        ...
        Ok(match guild_id {
            Some(guild_id) => (self.api_client.guild_member(guild_id, self.current_user).await?.model().await?, guild_id).into(),
