...
use crate::{luro_database_driver::LuroDatabaseItem, story::Story, Stories};

use super::{toml_deserializer, toml_serializer, TomlDatabaseDriver, NSFW_STORIES_FILE_PATH, SFW_STORIES_FILE_PATH};

impl LuroDatabaseItem for Story {
    type Item = Story;
    type Id = usize;
    ...
    async fn add_items(items: &Self::Container) -> anyhow::Result<()> {
        let sfw_path = Path::new(SFW_STORIES_FILE_PATH);
        let nsfw_path = Path::new(NSFW_STORIES_FILE_PATH);
    ...
    async fn get_item(id: &Self::Id, ctx: Self::Additional) -> anyhow::Result<Self::Item> {
        ...
        let data = toml_deserializer(TomlDatabaseDriver::get(path, Default::default()).await?)?;
    ...
    async fn remove_item(id: &Self::Id, ctx: Self::Additional) -> anyhow::Result<Option<Self::Item>> {
        ...
        let mut data = toml_deserializer(data)?;
