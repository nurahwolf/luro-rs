...
pub enum Error {
    ...
    TomlIOError(#[from] toml::de::Error),
    #[error("There was an error serializing the TOML content of the configuration")]
...
impl Config {
    ...
    pub async fn fetch(file_name: &str) -> Result<Self, Error> {
    ...
    pub async fn snyc_config(&mut self, new_config: Option<Self>, file_name: &str) -> Result<&mut Self, Error> {
        // Handle new config
        if let Some(new_config) = new_config {
            ...
            if let Err(why) = tokio::fs::write(file_name, toml_string).await {
                tracing::error!(
                    ?why,
                    "Failed to write toml_string to file. Changes will be lost as they have only been changed in memory!"
                );
            }
