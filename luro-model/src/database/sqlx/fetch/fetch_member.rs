...
impl Database {
    ...
    pub async fn fetch_member(&self, guild_id: Id<GuildMarker>, user_id: Id<UserMarker>) -> Result<MemberContext, Error> {
        match fetch_member(self, guild_id, user_id).await {
        ...
        Ok(self.twilight_driver.fetch_member(guild_id, user_id).await?)
    }
}

pub async fn fetch_member(db: &Database, guild_id: Id<GuildMarker>, user_id: Id<UserMarker>) -> Result<Option<MemberContext>, Error> {
    let g_id = guild_id.get() as i64;
    let u_id = user_id.get() as i64;

    // Fetch the member from the database
    let member = sqlx::query_file!("src/database/sqlx/queries/member/member_fetch.sql", g_id, u_id).fetch_optional(&db.pool);
