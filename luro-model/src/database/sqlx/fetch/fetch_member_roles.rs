...
impl Database {
    ...
    pub async fn fetch_member_roles<'a>(&'a self, member: &'a mut MemberContext) -> Result<&'a mut MemberContext, Error> {
        if !fetch_member_roles(self, member).await {
...
/// Fetches roles from the database and sets them to the member. Returns if roles were found
async fn fetch_member_roles(db: &Database, member: &mut MemberContext) -> bool {
    let g_id = member.guild_id.get() as i64;
    let u_id = member.user_id().get() as i64;
    let mut roles_found = false;
    let mut roles_query = sqlx::query_file!("src/database/sqlx/queries/member/member_fetch_roles.sql", g_id, u_id).fetch(&db.pool);
