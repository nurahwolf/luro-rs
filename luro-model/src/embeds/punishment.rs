use crate::{builders::EmbedBuilder, response::BannedResponse};

/// An embed sent to a user when they are banned
pub fn ban_user(data: &BannedResponse<'_>, guild_name: &str) -> EmbedBuilder {
...
pub fn ban_logged(data: &BannedResponse<'_>, dm_success: &bool) -> EmbedBuilder {
    ...
    match dm_success {
        true => description.push_str(&format!("<:private:1175114613172473987>`{purged_messages:^longest_word_length$}` `{DM_SUCCESS:^longest_word_length$}`<:mail:1175136204648349756>\n")),
        false => description.push_str(&format!("<:private:1175114613172473987>`{purged_messages:^longest_word_length$}` `{DM_FAILED:^longest_word_length$}`<:mail:1175136204648349756>\n")),
    };
