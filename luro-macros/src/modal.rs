...
use quote::ToTokens;
use syn::{parse2, spanned::Spanned, Data, DeriveInput, Error, Fields, FieldsNamed, Result, Type};
...
impl Field {
    ...
    fn parse(&mut self, attr: Attr) -> Result<()> {
        ...
        match attr.path.get_ident().unwrap().to_string().as_str() {
            ...
            "paragraph" => {
                if self.paragraph {
                    Err(Error::new(span, "paragraph already set"))?;
                } else if attr.has_values() {
                    Err(Error::new(span, "paragraph attribute doesn't admit values"))?;
                }
                self.paragraph = true;
            }
            "max_length" => {
...
pub fn modal(input: TokenStream2) -> Result<TokenStream2> {
    ...
    Ok(quote::quote! {
        const _: () = {
            ...
            impl<D> ::luro_framework::modal::Modal<D> for #struct_ident {
