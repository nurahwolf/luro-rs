...
pub fn parse_arguments<'a>(
    ...
    *block = parse2(quote::quote! {{
        let (#(#names),*) = {
            let __options = ::luro_framework::iter::DataIterator::new(#ctx_ident);
            ...
            if __options.len() > 0 {
                return Err(
                    ::luro_framework::parse::ParseError::StructureMismatch("Too many arguments received".to_string()).into()
                );
            }
