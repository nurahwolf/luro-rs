...
impl LuroCommand for Database {
    async fn interaction_command(self, ctx: CommandInteraction) -> anyhow::Result<luro_model::types::CommandResponse> {
        ...
        if let Ok(data) = ctx.database.sqlx.count_applications().await {
            builder.push_record(["Total Applications", &format_number(data)]);
        }
        if let Ok(data) = ctx.database.sqlx.count_channels().await {
            builder.push_record(["Total Channels", &format_number(data)]);
        }
        if let Ok(data) = ctx.database.sqlx.count_interactions().await {
            builder.push_record(["Total Interactions", &format_number(data)]);
        }

        if let Ok(word_count) = ctx.database.sqlx.count_messages().await
        ...
        if let Ok(messages) = ctx.database.sqlx.messages_count_word_totals().await {
        ...
        // Guild Data
        if let Ok(data) = ctx.database.sqlx.count_guilds().await {
            builder.push_record(["Total Guilds", &format_number(data)]);
        }
        if let Ok(data) = ctx.database.sqlx.count_guild_members().await {
            builder.push_record(["Total Guild Members", &format_number(data)]);
        }
        if let Ok(data) = ctx.database.sqlx.count_guild_roles().await {
        ...
        // User Data
        if let Ok(data) = ctx.database.sqlx.count_users().await {
            builder.push_record(["Total Users", &format_number(data)]);
        }
        if let Ok(data) = ctx.database.sqlx.count_user_characters().await {
            builder.push_record(["Total User Characters", &format_number(data)]);
        }
        if let Ok(data) = ctx.database.sqlx.count_user_moderation_actions().await {
            builder.push_record(["Total User Moderation Actions", &format_number(data)]);
        }
        if let Ok(data) = ctx.database.sqlx.count_user_warnings().await {
