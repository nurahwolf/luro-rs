...
#[poise::command(context_menu_command = "Add to favs", slash_command, category = "Favs", subcommands("get", "list", "change_category"))]
pub async fn favourites(ctx: Context<'_>, message: Message) -> Result<(), Error> {
    ...
    let uncategorised_favourites_length = match user_favourites {
        ...
        Entry::Vacant(vacant) => vacant.insert(new_favourite).len()
    };
