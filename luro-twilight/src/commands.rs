use luro_framework::{Framework, InteractionCommand, InteractionModal, InteractionComponent};
...
pub async fn handle_command<D: LuroDatabaseDriver>(framework: Framework<D>, interaction: InteractionCommand) -> anyhow::Result<()> {
    let data = interaction.data.clone();
    match data.name.as_str() {
        "say" => Say::new(data)?.run_command(framework, interaction).await,
...
pub async fn handle_component<D: LuroDatabaseDriver>(_ctx: Framework<D>, _interaction: InteractionComponent) -> anyhow::Result<()> {
...
pub async fn handle_autocomplete<D: LuroDatabaseDriver>(_ctx: Framework<D>, _interaction: InteractionCommand) -> anyhow::Result<()> {
    Ok(())
}
