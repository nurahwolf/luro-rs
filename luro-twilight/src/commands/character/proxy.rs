...
impl LuroCommand for Proxy {
    async fn interaction_command<D: LuroDatabaseDriver>(
        ...
        interaction.respond(&ctx, |response|response.embed(|embed|embed.author(|author|author.icon_url(character_icon).name(&self.name)).description("Your proxied messages will look like this now!\n\n*Note:* If I am using your avatar, make sure that I have been set with an icon! `/character icon`")).ephemeral()).await
    }
}
