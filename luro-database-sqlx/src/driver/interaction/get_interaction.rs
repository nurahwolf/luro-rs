...
impl crate::SQLxDriver {
    ...
    pub async fn get_interaction_by_message_id(&self, message_id: Id<MessageMarker>) -> anyhow::Result<Interaction> {
        ...
        Ok(Interaction {
            ...
            channel: Some(
                self.channel_fetch(Id::new(interaction.channel_id as u64))
                    .await?
                    .context("Expected to get channel from DB")?
                    .into(),
            ),
            ...
            message: match interaction.message_id {
                Some(message) => Some(
                    self.get_message(Id::new(message as u64))
                        .await?
                        .context("Should have returned message from database")?
                        .into(),
                ),
                None => None,
            },
