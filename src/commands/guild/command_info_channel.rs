...
use crate::{
    utils::{accent_colour, guild_accent_colour},
    Context, Error
};
...
pub async fn channel(
    ...
    embed.colour(accent_colour(ctx.data().config.accent_colour));
    ...
    if let Some(guild_channel) = ctx.serenity_context().cache.guild_channel(regular_channel.id()) {
        ...
        embed.colour(guild_accent_colour(ctx.data().config.accent_colour, Some(guild_resolved.to_owned())));
